...
class View:
    ...
    def add_visualizations(self):
        for visual in self.model.get_visualizations().values():
            ...
            if visual.get_id() == 4:
                ...
                interior = self.add_scrollbar(right_frame)

                if visual.get_id() == 1:
                    self.add_checkboxes(visual, interior, self.bubble_counties, figure_frame)
                if visual.get_id() == 2:
                    self.add_checkboxes(visual, interior, self.column_counties, figure_frame)
                if visual.get_id() == 3:
                    self.add_checkboxes(visual, interior, self.pie_counties, figure_frame)
    ...
    def add_checkboxes(self, visual, interior, dict, figure_frame):
        for county in visual.get_counties():
            checkbox = tk.IntVar(value=1)
            dict[county] = checkbox  
            button = tk.Checkbutton(interior, text = county, 
                            variable = checkbox, 
                            onvalue = 1, 
                            offvalue = 0,
